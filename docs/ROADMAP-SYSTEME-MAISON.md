...
- **2026-02-20** (suite) : **Historique terminal par session** : backend lab-terminal attribue un ID unique (`anon-<hex>`) si `session` est vide pour ne jamais partager de buffer ; frontend `getTerminalUrl` inclut toujours un `session` (génération `s`+timestamp+random si vide). **Simulateur réseau** : persistance de la carte courante avant **changement de lab** (ref `lastSimStateRef` + sauvegarde du lab quitté dans l’effet `effectiveLabId`). **Tests** : test 11 vérifie que le store fournit une URL avec paramètre `session`.
